...
if (is_post()) {
    if (!verify_csrf_token($_POST[CSRF_TOKEN_NAME] ?? null)) {
        ...
        if ($action === 'create_item') {
            ...
            if (!$errors) {
                try {
                    $stmt = $appsPdo->prepare('INSERT INTO inventory_items (sku, name, sector_id, quantity, location) VALUES (:sku, :name, :sector_id, :quantity, :location)');
                    $stmt->execute([
                        ':sku' => $sku !== '' ? $sku : null,
                        ':name' => $name,
                        ':sector_id' => $sectorId,
                    ...
                    if ($quantity > 0) {
                        $movStmt = $appsPdo->prepare('INSERT INTO inventory_movements (item_id, direction, amount, reason, user_id) VALUES (:item_id, :direction, :amount, :reason, :user_id)');
                        $movStmt->execute([
                            ':item_id' => $itemId,
                            ':direction' => 'in',
                            ':amount' => $quantity,
            ...
            if (!$item) {
                ...
                if (!$errors) {
                    try {
                        $updStmt = $appsPdo->prepare('UPDATE inventory_items SET name=:name, sku=:sku, location=:location, sector_id=:sector_id WHERE id=:id');
                        $updStmt->execute([
                            ':name' => $name,
                            ':sku' => $sku !== '' ? $sku : null,
                            ':location' => $location !== '' ? $location : null,
            ...
            if (!$item) {
                ...
                if ($newQuantity < 0) {
                    ...
                    try {
                        ...
                        $appsPdo->prepare('UPDATE inventory_items SET quantity = quantity + :delta WHERE id = :id')->execute([
                            ':delta' => $delta,
                            ':id' => $itemId,
                        ]);
                        $appsPdo->prepare('INSERT INTO inventory_movements (item_id, direction, amount, reason, user_id) VALUES (:item_id, :direction, :amount, :reason, :user_id)')->execute([
                            ':item_id' => $itemId,
                            ':direction' => $direction,
                            ':amount' => $amount,
        ...
        <label>Sector
            <select name="sector" <?php echo $isRoot ? '' : 'disabled'; ?>>
                ...
                <?php foreach ($sectorOptions as $sector): ?>
                    <option value="<?php echo (int)$sector['id']; ?>" <?php echo ((string)$sector['id'] === (string)$sectorFilter) ? 'selected' : ''; ?>><?php echo sanitize($sector['name']); ?></option>
                <?php endforeach; ?>
            </select>
        </label>
        <?php if ($isRoot): ?>
    ...
    <table class="table">
        ...
        <?php foreach ($items as $item): ?>
            <tr>
                ...
                <td><?php echo $item['sku'] ? sanitize($item['sku']) : '<em>-</em>'; ?></td>
                <td>
                    <?php
                    $name = '';
                    foreach ($sectorOptions as $sector) {
                        if ($sector['id'] == $item['sector_id']) { $name = $sector['name']; break; }
                    }
                    echo $name ? sanitize($name) : '<em>Unassigned</em>';
                    ?>
                </td>
                <td><?php echo (int)$item['quantity']; ?></td>
                <td><?php echo $item['location'] ? sanitize($item['location']) : '<em>-</em>'; ?></td>
                <td>
                    <details>
                        ...
                        <?php if ($canManage && ($isRoot || (int)$item['sector_id'] === (int)$userSectorId)): ?>
                            <form method="post" class="form-inline">
                                ...
                                <label>SKU
                                    <input type="text" name="sku" value="<?php echo sanitize($item['sku'] ?? ''); ?>">
                                </label>
                                <label>Location
                                    <input type="text" name="location" value="<?php echo sanitize($item['location'] ?? ''); ?>">
                                </label>
                                <?php if ($isRoot): ?>
                                    <label>Sector
                                        <select name="sector_id">
                                            <option value="null">Unassigned</option>
                                            <?php foreach ($sectorOptions as $sector): ?>
                                                <option value="<?php echo (int)$sector['id']; ?>" <?php echo ($item['sector_id'] == $sector['id']) ? 'selected' : ''; ?>><?php echo sanitize($sector['name']); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </label>
                                <?php endif; ?>
                        ...
                        <ul class="movements">
                            <?php foreach ($movementsByItem[$item['id']] ?? [] as $move): ?>
                                <li>
                                    <strong><?php echo sanitize(strtoupper($move['direction'])); ?></strong>
                                    <?php echo (int)$move['amount']; ?>
                                    <span class="muted small"><?php echo sanitize($move['ts']); ?></span>
                                    <?php if ($move['reason']): ?> - <?php echo sanitize($move['reason']); ?><?php endif; ?>
                                </li>
                            <?php endforeach; ?>
